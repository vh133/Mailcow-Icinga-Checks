...
WARNING = 1
CRITICAL = 2
usage = 'usage: ./check_mailqueue.py -w/--warn <integer> -c/--crit <integer> -a/--apiurl <string> -k/--apikey <string>'
...
def check_mailqueue(apiurl, apikey):
  try:
    ...
    print('ERROR in check_mailqueue')
...
def main():
  ...
  counter = check_mailqueue(apiurl, apikey)
  if counter >= crit:
    print('CRITICAL: ' + str(counter) + ' Mails in queue')
    sys.exit(CRITICAL)
  elif counter >= warn:
    print('WARNING: ' + str(counter) + ' Mails in queue')
    sys.exit(WARNING)
  elif counter < warn:
    print('OK: ' + str(counter) + ' Mails in queue')
    sys.exit(OK)
  else:
